  ...
  factory LatLng.fromJson(final Map<String, dynamic> json) {
    ...
    if (json.containsKey("type") && json["type"] == "Point") {
      if (json.containsKey("coordinates") && json["coordinates"] is List) {
        return LatLng(
            lat: json["coordinates"][1] is String
                ? num.parse(json["coordinates"][1]).toDouble()
                : (json["coordinates"][1] as num).toDouble(),
            lng: json["coordinates"][0] is String
                ? num.parse(json["coordinates"][0]).toDouble()
                : (json["coordinates"][0] as num).toDouble());
      } else {
        throw ArgumentError(
            "Type point missing field coordinates or is not array!");
      }
    } else {
      throw ArgumentError("GeoJson is not point!");
    }
  }
